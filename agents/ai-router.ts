...
export class AIRouter {
  ...
  async useGroqJSON<T>(messages: GroqMessage[], taskType: GroqTaskType = 'structured_output'): Promise<{ success: boolean; data?: T; error?: string }> {
    this.groqRequests++;
    this.totalRequests++;
    return this.groq.chatJSON(messages, taskType);
  }

  async useGeminiJSON<T>(messages: GeminiMessage[], taskType: GeminiTaskType = 'general'): Promise<{ success: boolean; data?: T; error?: string }> {
    this.geminiRequests++;
    this.totalRequests++;
