...
export class ThemeDesigner {
  ...
  async designTheme(request: ThemeDesignRequest): Promise {
...
- Think like a $10,000 agency designer`;
  ...
  private async generateCSS(design: ThemeDesign): Promise {
...
`;
  ...
  private async applyToStore(design: ThemeDesign): Promise {
    try {
      // Get main theme
  ...
  async modifyTheme(modification: string): Promise {
...
Only include keys that need to change. Keep everything else the same.`;
    ...
    const response = await this.router.useGeminiJSON<{
      newMood: string;
      colorChanges?: Partial;
      fontChanges?: Partial;
      spacingChanges?: Partial;
      styleChanges?: Partial;
  ...
  async applyEventTheme(event: string): Promise {
    const eventMoods: Record = {
  ...
  async removeCustomTheme(): Promise {
    try {
