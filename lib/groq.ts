...
const TASK_CONFIGS: Record<groqtasktype,>> = {
  code_generation: {
...
export class GroqClient {
  ...
  async planShopifyAction(task: string, currentState?: string): Promise {
    const messages: GroqMessage[] = [];
    if (currentState) {
      messages.push({
        role: 'user',
...
Return JSON with: sellingPrice, compareAtPrice, actualMargin, priceJustification`;
  ...
  async structureData(input: string, outputFormat: string): Promise {
...
Return ONLY valid JSON, no markdown, no explanation.`;
  ...
  async quickAnswer(question: string): Promise {
...
Write compelling, conversion-focused content. No filler words.`;
  ...
  async analyzeData(data: string, question: string): Promise {
