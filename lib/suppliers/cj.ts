...
export class CJDropshippingSupplier extends BaseSupplier {
  ...
  constructor(config?: Partial<SupplierConfig>) {
    super({
  ...
  async getProducts(externalIds: string[]): Promise<SupplierProduct[]> {
  ...
  async checkPrice(externalId: string): Promise<PriceCheck> {
  ...
  async checkPrices(externalIds: string[]): Promise<PriceCheck[]> {
  ...
  async placeOrder(items: SupplierOrderItem[], shippingAddress: ShippingAddress): Promise<SupplierOrder> {
  ...
  async getOrderStatus(externalOrderId: string): Promise<SupplierOrder | null> {
    const response = await this.apiRequest(
      'GET',
  ...
  async getShippingEstimate(externalId: string, destination: string): Promise<ShippingEstimate[]> {
    const body = {
      startCountryCode: 'CN',
  ...
  async getProductVariants(productId: string): Promise<SupplierVariant[]> {
  ...
  private mapOrderStatus(cjStatus: string): SupplierOrder['status'] {
    const statusMap: Record<string, SupplierOrder['status']> = {
      'CREATED': 'pending',
      'IN_CART': 'pending',
      'UNPAID': 'pending',
      'UNSHIPPED': 'confirmed',
...
export function getCJSupplier(config?: Partial<SupplierConfig>): CJDropshippingSupplier {
  if (!cjInstance) {
