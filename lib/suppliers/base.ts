...
export abstract class BaseSupplier {
   ...
   */
  abstract searchProducts(params: SupplierSearchParams): Promise<SupplierSearchResult>;

  /**
   * Get single product details by external ID
   */
  abstract getProduct(externalId: string): Promise<SupplierProduct | null>;

  /**
   * Get multiple products by IDs (batch)
   */
  abstract getProducts(externalIds: string[]): Promise<SupplierProduct[]>;

  /**
   * Check current price of a product
   */
  abstract checkPrice(externalId: string): Promise<PriceCheck>;

  /**
   * Check prices for multiple products (batch)
   */
  abstract checkPrices(externalIds: string[]): Promise<PriceCheck[]>;

  /**
   * Place order with supplier
   */
  abstract placeOrder(items: SupplierOrderItem[], shippingAddress: ShippingAddress): Promise<SupplierOrder>;

  /**
   * Get order status from supplier
   */
  abstract getOrderStatus(externalOrderId: string): Promise<SupplierOrder | null>;

  /**
   * Get tracking information
   */
  abstract getTracking(externalOrderId: string): Promise<TrackingInfo | null>;

  /**
   * Get shipping estimates for a product to destination
   */
  abstract getShippingEstimate(externalId: string, destination: string): Promise<ShippingEstimate[]>;
...
export type SupplierFactory = (config: Partial<SupplierConfig>) => BaseSupplier;
