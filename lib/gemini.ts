...
const TASK_CONFIGS: Record<geminitasktype,>> = {
  design_decision: {
...
export class GeminiClient {
  ...
  async interpretUrdu(message: string): Promise {
...
Return JSON:
{
  ...
  async interpretMood(moodDescription: string): Promise {
...
Return JSON:
{
  ...
  async complexReasoning(problem: string, context?: string): Promise {
  ...
  async chatJSON(
    ...
    try {
      ...
      const fixResponse = await this.chat(
        ...
        'structured_output' as GeminiTaskType
      );
