...
class KVAdapter {
  ...
  private kv: { get: (key: string) => Promise; set: (key: string, value: string) => Promise } | null = null;

  constructor() {
    this.initKV();
  }

  private async initKV(): Promise {
    try {
      if (process.env.KV_REST_API_URL && process.env.KV_REST_API_TOKEN) {
        const { kv } = await import('@vercel/kv');
        this.kv = kv as unknown as { get: (key: string) => Promise; set: (key: string, value: string) => Promise };
...
export class MemoryManager {
  ...
  async addMessage(message: Omit): Promise {
  ...
  async getConversationHistory(limit: number = 20): Promise {
    const memory = await this.load();
    return memory.conversations.slice(-limit);
  }

  async getRecentContext(messageCount: number = 10): Promise {
  ...
  async logAction(action: Omit): Promise {
    const memory = await this.load();
  ...
  async getActionsByAgent(agent: string, limit: number = 10): Promise {
    const memory = await this.load();
  ...
  async getLastAction(agent?: string): Promise {
    const memory = await this.load();
    if (agent) {
  ...
  async getReversibleActions(): Promise {
    const memory = await this.load();
  ...
  async getPreference(key: K): Promise {
    const memory = await this.load();
  ...
  async updateStoreState(updates: Partial): Promise {
    const memory = await this.load();
    memory.storeState = {
      ...memory.storeState,
      ...updates,
  ...
  async createSnapshot(label: string, additionalData?: { themeCSS?: string; homepageConfig?: unknown; productPageConfig?: unknown }): Promise {
    const memory = await this.load();
  ...
  async setContext(key: string, value: unknown): Promise {
    const memory = await this.load();
    memory.context[key] = value;
    this.debouncedSave();
  }

  async getContext(key: string): Promise {
    const memory = await this.load();
    return memory.context[key] || null;
  }

  async deleteContext(key: string): Promise {
    const memory = await this.load();
  ...
  async getAgentContext(agentName: string): Promise {
    const memory = await this.load();
  ...
  async resetConversations(): Promise {
    const memory = await this.load();
    memory.conversations = [];
    await this.save();
  }

  async resetActions(): Promise {
    const memory = await this.load();
    memory.actions = [];
    await this.save();
  }
